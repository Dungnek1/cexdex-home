...
const CARDS = [
  { tag: 'Spot Trading', title: 'Instant crypto trading at transparent market prices', icon: MARKET_ICONS.spot },
  { tag: 'Future Trading', title: 'Leverage trades to profit in rising and falling markets', icon: MARKET_ICONS.future },
  { tag: 'AI Bot', title: 'Activate AI bots to trade automatically 24/7', icon: MARKET_ICONS.aiBot },
]

function MarketCard({ tag, title, icon }: { tag: string; title: string; icon: string }) {
  return (
    <div
    ...
    >
      <div className="flex flex-col gap-2 tablet:gap-[14px]">
        <div className="flex gap-2 tablet:gap-[10px] items-center">
          <span
            className="w-2.5 h-2.5 tablet:w-3 tablet:h-3 shrink-0 rotate-45 bg-[var(--gray-10,#f1f1f1)]"
            aria-hidden
          />
          <p className="text-[16px] tablet:text-[20px] leading-[24px] tablet:leading-[30px] font-extrabold whitespace-nowrap shrink-0 font-manrope text-[var(--gray-10,#f1f1f1)]">
            {tag}
          </p>
        </div>
        <p className="text-[18px] tablet:text-[24px] leading-[26px] tablet:leading-[32px] font-normal w-full text-[var(--gray-50,#939393)]">
          {title}
        </p>
      </div>

      <div
        className="flex items-center justify-between pb-[12px] border-b border-solid cursor-pointer"
        style={{ borderColor: 'var(--gray-80, #484848)' }}
      >
        <span className="text-[16px] font-semibold leading-[24px] font-manrope text-[var(--gray-10,#f1f1f1)]">
          Custody Now
        </span>
    ...
    >
      ...
      >
        ...
        <div className="market-platform-header-content absolute inset-0 flex items-center justify-center overflow-hidden">
          <span
            className="market-platform-label absolute left-0 right-0 text-[40px] tablet:text-[80px] lg:text-[120px] font-normal whitespace-nowrap tracking-[-1.6px] tablet:tracking-[-3.2px] lg:tracking-[-4.8px] leading-none text-center gradient-heading"
            style={{
      ...
      <div className="w-full section-px overflow-x-auto tablet:overflow-visible lg:overflow-visible scroll-smooth scrollbar-hidden">
