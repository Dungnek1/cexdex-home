    ...
    <section>
      <div className="w-full section-px">
        <div className="flex flex-col gap-[10px] tablet:gap-[40px] lg:flex-row lg:items-start lg:gap-[120px]">
          <div className="pt-[6px] lg:w-[226px] lg:shrink-0">
            <SectionHeader title="Platform Results" />
          </div>

          <div className="flex-1 min-w-0 flex flex-col gap-5 tablet:gap-[50px] lg:gap-[70px]">
            {STATS.map((row, ri) => (
              <div key={ri} className="grid grid-cols-2 tablet:grid-cols-2 gap-x-[30px] tablet:gap-x-[40px] gap-y-5 tablet:gap-y-0">
                {row.map((stat) => (
